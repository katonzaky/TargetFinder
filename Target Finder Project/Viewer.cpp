...
void Viewer::reset()
{
    glm::vec3 at(centerX, centerY, centerZ);
    glm::vec3 up(0.0f, 1.0f, 0.0f);
    viewMtx = glm::lookAt(initEye, at, up);
}

void Viewer::center(float x, float y, float z)
{
	centerX = x;
	centerY = y;
	centerZ = z;
...
void ObjectViewer::update( InputState &input ) 
{
    ...
    if ( input.rMousePressed )
    {
        ...
        if ( xRot != 0 )
        {
        	if (xRot < 0)
        	{
		    	eyeX = eyeX * (1.01f + (abs(xRot) * 0.001f));
		        eyeY = eyeY * (1.01f + (abs(xRot) * 0.001f));
		        eyeZ = eyeZ * (1.01f + (abs(xRot) * 0.001f));
        	}
        	if (xRot > 0)
        	{
		    	eyeX = eyeX * (0.98f - (abs(xRot) * 0.001f));
		        eyeY = eyeY * (0.98f - (abs(xRot) * 0.001f));
		        eyeZ = eyeZ * (0.98f - (abs(xRot) * 0.001f));
        	}
...
FirstPerson::FirstPerson( glm::vec3 eye ) : Viewer (eye)
{
...
void FirstPerson::update( InputState &input ) 
{
}
